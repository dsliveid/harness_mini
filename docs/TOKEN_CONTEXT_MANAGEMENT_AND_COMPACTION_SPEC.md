...
| 阵营 / 分类 | 模型规格标识 (Preset ID) | 代表厂商 / 模型 | 物理窗口总量 | 推荐上下文阈值 (Recommended Limit) | 预留补全/输出空间 |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **海外阵营** | **`gpt-5-5`** | OpenAI GPT-5.5 | 1,000,000 | **800,000** | 200,000 |
| **海外阵营** | **`claude-opus-5`** | Anthropic Claude Opus 5 | 1,000,000 | **800,000** | 200,000 |
| **海外阵营** | **`gemini-3-8-flash`** | Google Gemini 3.8 Flash | 1,000,000 | **800,000** | 200,000 |
| **海外阵营** | **`openai-o1`** | OpenAI o1 / o3 / o3-mini | 200,000 | **180,000** | 20,000 |
| **海外阵营** | **`claude-3-5`** | Anthropic Claude 3.5 / 3.7 Sonnet | 200,000 | **180,000** | 20,000 |
| **海外阵营** | **`gpt-4o`** | OpenAI GPT-4o / GPT-4o-mini | 128,000 | **110,000** | 18,000 |
| **国产阵营** | **`deepseek-v4-flash`** | 深度求索 DeepSeek-v4-flash (1M) | 1,000,000 | **800,000** | 200,000 |
| **国产阵营** | **`qwen-3-8-flash`** | 阿里通义千问 Qwen3.8-flash (1M) | 1,000,000 | **800,000** | 200,000 |
| **国产阵营** | **`glm-5-3-flash`** | 智谱清言 GLM-5.3-flash (1M) | 1,000,000 | **800,000** | 200,000 |
| **国产阵营** | **`kimi-1m`** | 月之暗面 Moonshot Kimi (1M) | 1,000,000 | **800,000** | 200,000 |
| **国产阵营** | **`deepseek-128k`** | DeepSeek V3 / R1 (128K 标准) | 128,000 | **110,000** | 18,000 |
| **国产阵营** | **`qwen-2-5`** | 阿里通义千问 Qwen 2.5 / Plus | 128,000 | **110,000** | 18,000 |
| **国产阵营** | **`glm-4`** | 智谱清言 GLM-4 / Plus | 128,000 | **110,000** | 18,000 |
| **国产阵营** | **`deepseek-64k`** | DeepSeek (64K 经典) | 64,000 | **56,000** | 8,000 |
| **本地模型** | **`ollama-32k`** | 本地常规模型 (Ollama 32K) | 32,768 | **28,000** | 4,768 |
| **本地模型** | **`ollama-16k`** | 本地常规模型 (Ollama 16K) | 16,000 | **14,000** | 2,000 |
| **本地模型** | **`ollama-8k`** | 本地小显存模型 (Llama 8B 默认) | 8,192 | **7,000** | 1,192 |

### 2. 界面交互与手动设置能力
- **模型条目独立微调**：设置中心（`SettingsModal.tsx`）各厂商模型列表中，每个模型右侧均展示有对应的上下文上限标签（如 `800k`），点击即可弹出专属规格配置弹窗微调该模型的数值。
- **会话级专属微调**：顶栏模型下拉菜单与会话设置（`SessionSettingsModal.tsx`）中提供「上下文上限」专属配置页，可针对当前对话单独自定义上限（如临时提升到 1M 或收窄防超限），不影响全局与其他对话。
- **开放式手动微调**：数字输入框放宽至 `2,000` 到 `10,000,000` Tokens，全面支持超长文本模型与百万级上下文。
- **未识别模型保底上限**：未匹配到任何推断规则且用户未单独指定时，保底使用 `64,000` Tokens。
