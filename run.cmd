@echo off
rem harness_mini dev launcher for Windows.
rem
rem Usage: run.cmd [options]     (options are forwarded to run.sh, e.g. --dry-run)
rem
...
rem of run.sh renders correctly, and restores the previous code page on exit.
rem
...
rem remember current code page, switch to UTF-8 for run.sh output
set "OLDCP="
for /f "tokens=2 delims=:" %%A in ('chcp') do set "OLDCP=%%A"
chcp 65001 >nul

"%BASH_EXE%" "%HERE%run.sh" %*
set "RC=%ERRORLEVEL%"
