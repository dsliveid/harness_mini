...
export function SubagentView({ subagentId }: { subagentId: string }) {
  ...
  const tokenStats = useMemo(() => {
    ...
    const finalCached = Math.max(cached, Number(subagent?.cachedTokens) || 0);
  ...
  }, [msgs, isRunning, subagent?.totalTokens, subagent?.promptTokens, subagent?.completionTokens, subagent?.cachedTokens]);
    ...
    <div className="flex-1 flex flex-col min-h-0 bg-panel border-l border-edge relative z-10">
      ...
      >
        <div className="flex items-center gap-1.5 min-w-0 shrink overflow-hidden">
          ...
          {subagent.workspacePath && !isCompact && (
            <span
              className="hidden lg:inline-flex items-center gap-1 h-8 px-2 rounded-lg text-[11px] text-inkdim bg-panel2/80 border border-edge/60 max-w-[180px] truncate shrink-0 cursor-default"
      ...
      <div className="p-3">
        ...
        <div className="h-6 flex items-center justify-between text-[11px] text-inkdim mt-2 px-1 select-none gap-3">
          ...
          <div className="flex items-center gap-2 shrink-0">
            {/* 上下文长度消耗圆圈进度指示器 */}
            <ContextUsageGauge session={subagent} />
